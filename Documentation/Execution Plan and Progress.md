...
- `TorrentCore.WebUI` is now the supported operator client; `TorrentCore.Web` and `TorrentCore.Avalonia` have been deleted from the repo
- historical progress notes below that mention Avalonia or the earlier Web UI remain in this document for delivery history, not as the current repo shape or product direction
...
- completion-callback finalization now also considers MonoTorrent's current active file path (`ITorrentManagerFile.FullPath`): if MonoTorrent has already promoted a file to its complete path, a lingering `.!mt` sibling is treated as stale instead of still-active, but TorrentCore no longer deletes partial or payload-side files during callback finalization
- MonoTorrent outbound peer policy is now operator-selectable through `Engine Encryption Mode`, defaults to `EncryptedPreferred`, restores IPv6 listening when the host supports it, and logs explicit `peer_connected` / `peer_disconnected` events with peer URI, client, direction, and encryption so live metadata stalls can be distinguished from connection-establishment failures
- the Settings UI and runtime settings API now expose `PlainTextPreferred`, `EncryptedPreferred`, and `EncryptedRequired`; this was added after a real swarm showed that plaintext-first negotiation could leave TorrentCore stuck while Transmission completed immediately with encrypted-preferred peers
...
- MonoTorrent shutdown now flushes live manager state back to SQLite before managers are stopped and cleared, which closes a timing window where a torrent could visibly reach 100% but restart from stale persisted progress after an OS-controlled service stop/start
- TorrentCore no longer performs proactive stale-partial cleanup during completion callback finalization; this avoids a rename-race hazard where a final media file could disappear immediately after MonoTorrent promoted `*.mkv.!mt` to `*.mkv`. Cleanup of non-media residue is left to the downstream callback app or operator policy, while TorrentCore only waits, invokes, and reports callback state
