...
__attribute__((used)) static const char kLauncherIdentity[] =
    "com.conadv.torrentcore.service.launcher";
...
static int install_signal_handlers(void)
{
    ...
    for (size_t index = 0; index < sizeof(signals) / sizeof(signals[0]); index++) {
        if (sigaction(signals[index], &action, NULL) != 0) {
            fprintf(stderr, "TorrentCoreService launcher could not install signal handler %d: %s\n",
...
static char *path_relative_to_executable(const char *executable_path, const char *relative_path)
{
    ...
    *separator = '\0';
    ...
    if (candidate != NULL) {
        (void) snprintf(candidate, length, "%s/%s", directory, relative_path);
    }
    free(directory);
    if (candidate == NULL) {
        return NULL;
...
static int register_bundle(const char *executable_path)
{
    ...
    if (bundle_path == NULL) {
        fprintf(stderr, "TorrentCoreService launcher could not resolve its bundle.\n");
        return 1;
    }
    ...
    free(bundle_path);
    if (bundle_url == NULL) {
        fprintf(stderr, "TorrentCoreService launcher could not create its bundle URL.\n");
        return 1;
    }
    OSStatus status = LSRegisterURL(bundle_url, true);
    CFRelease(bundle_url);
    if (status != noErr) {
        fprintf(stderr, "TorrentCoreService bundle registration failed with status %d.\n", (int) status);
        return 1;
    }
    return 0;
}
...
static int run_helper(const char *helper_path, const char *working_directory, int argc, char **argv)
{
    ...
    pid_t pid = fork();
    if (pid < 0) {
        fprintf(stderr, "TorrentCoreService launcher could not create its child process: %s\n", strerror(errno));
        free(helper_argv);
        return 1;
    }
    if (pid == 0) {
        if (chdir(working_directory) != 0) {
            fprintf(stderr, "TorrentCoreService launcher could not enter %s: %s\n",
                    working_directory, strerror(errno));
            _exit(126);
        }
        execv(helper_path, helper_argv);
        fprintf(stderr, "TorrentCoreService launcher could not execute %s: %s\n",
    ...
    while (waitpid(pid, &status, 0) < 0) {
        if (errno != EINTR) {
            fprintf(stderr, "TorrentCoreService launcher could not wait for its child: %s\n", strerror(errno));
            child_pid = -1;
            free(helper_argv);
            return 1;
        }
...
int main(int argc, char **argv)
{
    ...
    if (executable_path == NULL) {
        fprintf(stderr, "TorrentCoreService launcher could not resolve its executable path.\n");
        return 1;
    }
    ...
    char *helper_path = path_relative_to_executable(
        executable_path, "../Resources/Runtime/TorrentCoreService");
    const char *working_directory = getenv("TORRENTCORE_WORKING_DIRECTORY");
    ...
    if (helper_path == NULL || working_directory == NULL || access(helper_path, X_OK) != 0) {
        fprintf(stderr, "TorrentCoreService runtime helper is missing or not executable.\n");
        free(helper_path);
        return 1;
    }
