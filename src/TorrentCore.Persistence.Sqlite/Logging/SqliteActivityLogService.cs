...
public sealed class SqliteActivityLogService(string databaseFilePath, int maxEntryCount) : IActivityLogService
{
    ...
    public async Task WriteAsync(ActivityLogWriteRequest request, CancellationToken cancellationToken)
    {
        ...
        command.Parameters.AddWithValue("$occurred_at_utc", DateTimeOffset.UtcNow.ToString("O", CultureInfo.InvariantCulture));
        ...
        command.Parameters.AddWithValue("$service_instance_id", request.ServiceInstanceId?.ToString() ?? (object)DBNull.Value);
    ...
    public async Task<IReadOnlyList<ActivityLogEntry>> GetRecentAsync(ActivityLogQuery query, CancellationToken cancellationToken)
    {
        await EnsureInitializedAsync(cancellationToken);
        ...
        if (query.FromUtc is not null)
        {
            ...
            command.Parameters.AddWithValue("$from_utc", query.FromUtc.Value.ToString("O", CultureInfo.InvariantCulture));
        }
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            results.Add(new ActivityLogEntry
            {
                ...
                OccurredAtUtc  = DateTimeOffset.Parse(reader.GetString(1), CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind),
                Level          = Enum.Parse<ActivityLogLevel>(reader.GetString(2), ignoreCase: true),
