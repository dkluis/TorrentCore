...
public sealed class TorrentCoreApiAdapter(TorrentCoreClient client) : ITorrentCoreApiAdapter
{
    ...
    public Task<ServiceCallResult<IReadOnlyList<TorrentHistorySummaryDto>>> GetHistoryAsync(
        TorrentHistoryQueryRequest request,
        CancellationToken cancellationToken = default
    )
        => ExecuteAsync(() => client.GetHistoryAsync(request, cancellationToken));

    public Task<ServiceCallResult<TorrentHistoryDetailDto?>> GetHistoryByTorrentIdAsync(
        Guid torrentId,
        CancellationToken cancellationToken = default
    )
        => ExecuteAsync(() => client.GetHistoryByTorrentIdAsync(torrentId, cancellationToken));
    ...
    public Task<ServiceCallResult<CleanupByDateResultDto>> CleanupLogsAsync(
        CleanupByDateRequest request,
        CancellationToken cancellationToken = default
    )
        => ExecuteAsync(() => client.CleanupLogsAsync(request, cancellationToken));

    public Task<ServiceCallResult<CleanupByDateResultDto>> CleanupHistoryAsync(
        CleanupByDateRequest request,
        CancellationToken cancellationToken = default
    )
        => ExecuteAsync(() => client.CleanupHistoryAsync(request, cancellationToken));
