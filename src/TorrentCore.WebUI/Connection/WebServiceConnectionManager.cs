...
public sealed class WebServiceConnectionManager
{
    ...
    public Task<TorrentCoreConnectionProbeResult> TestAsync(
        string? baseUrl,
    ...
    public async Task<TorrentCoreConnectionProbeResult> SaveAsync(
        string? baseUrl,
        CancellationToken cancellationToken = default
    )
    {
    ...
    private async Task EnsureInitializedAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (CurrentStatus.IsReachable && !string.IsNullOrWhiteSpace(CurrentStatus.BaseUrl))
            {
                _endpointProvider.Update(CurrentStatus.BaseUrl);
            }
            _initialized = true;
        }
        finally
