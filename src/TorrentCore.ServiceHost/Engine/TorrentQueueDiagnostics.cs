...
internal static class TorrentQueueDiagnostics
{
        ...
        RuntimeSettingsSnapshot runtimeSettings)
    {
        var diagnostics = snapshots.ToDictionary(
            snapshot => snapshot.TorrentId, _ => new TorrentQueueDiagnostic(null, null)
        );
        ...
        for (var index = 0; index < metadataQueue.Length; index++)
        {
            diagnostics[metadataQueue[index].TorrentId] = new TorrentQueueDiagnostic(
                index < availableMetadataSlots ? TorrentWaitReason.PendingMetadataDispatch :
                        TorrentWaitReason.WaitingForMetadataSlot, index + 1
            );
        }
        ...
        for (var index = 0; index < downloadQueue.Length; index++)
        {
            diagnostics[downloadQueue[index].TorrentId] = new TorrentQueueDiagnostic(
                index < availableDownloadSlots ? TorrentWaitReason.PendingDownloadDispatch :
                        TorrentWaitReason.WaitingForDownloadSlot, index + 1
            );
        }
