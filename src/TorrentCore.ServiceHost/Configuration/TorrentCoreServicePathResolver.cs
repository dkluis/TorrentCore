...
public static class TorrentCoreServicePathResolver
{
    public static ResolvedTorrentCoreServicePaths Resolve(string contentRootPath, TorrentCoreServiceOptions options)
    {
        return new ResolvedTorrentCoreServicePaths
        {
            ...
            DatabaseFilePath = ResolveAbsolutePath(
                contentRootPath, Path.Combine(options.StorageRootPath, "torrentcore.db")
            ),
        };
    }
    ...
    {
        var path = Path.IsPathRooted(configuredPath) ? configuredPath : Path.Combine(contentRootPath, configuredPath);
