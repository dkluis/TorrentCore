...
public sealed class TorrentStartupRecoveryService(
    ...
    ILogger<TorrentStartupRecoveryService> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var change in result.Changes)
        {
            await activityLogService.WriteAsync(new ActivityLogWriteRequest
            {
                ...
                Message = $"Normalized torrent '{change.Name}' from '{change.PreviousState}' to '{change.CurrentState}' during startup recovery.",
