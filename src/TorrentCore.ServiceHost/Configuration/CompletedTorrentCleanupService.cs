...
public sealed class CompletedTorrentCleanupService(
    ...
    ILogger<CompletedTorrentCleanupService> logger) : BackgroundService
{
    ...
    private async Task ProcessTickAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var candidates = torrents
                .Where(torrent => torrent.State == TorrentState.Completed && torrent.CompletedAtUtc is not null)
                .Where(torrent => (now - torrent.CompletedAtUtc!.Value).TotalMinutes >= runtimeSettings.CompletedTorrentCleanupMinutes)
            ...
            foreach (var torrent in candidates)
            {
                try
                {
                    await torrentEngineAdapter.RemoveAsync(torrent.TorrentId, new RemoveTorrentRequest
                    {
                        DeleteData = false,
                    }, cancellationToken);
                    ...
                    await activityLogService.WriteAsync(new ActivityLogWriteRequest
                    {
                        ...
                        Message = $"Automatically removed completed torrent '{torrent.Name}' from TorrentCore tracking.",
                ...
                {
                    logger.LogDebug("Completed torrent {TorrentId} was already removed before cleanup executed.", torrent.TorrentId);
                }
                catch (Exception exception)
                {
                    logger.LogWarning(exception, "Automatic cleanup failed for completed torrent {TorrentId}", torrent.TorrentId);
